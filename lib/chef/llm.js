...
// rules apply here too, so the exported API is typed via JSDoc (and mirrored in
// llm.d.ts for the TS side) rather than `any`.
 ...
 * @property {string[]} includeIngredients - Ingredients (canonicalised) the user wants included.
 ...
 * @returns {boolean}
 */
function recipeHasIngredient(recipe, needle) {
  return recipe.ingredients.some((ingredient) => {
    const words = normalize(ingredient.item).split(" ");
    return words.some((word) => fuzzyEqual(canonical(word), needle));
  });
}
...
function extractIncludes(normalized) {
  /** @type {Set<string>} */
  const found = new Set();
  const regex = /\b(?:with|using|use|contains?|containing|got|have|made from|from)\s+([a-z0-9\s-]+)/g;
  let match;
...
export { parseQuery, rankRecipes, suggestRecipes };
