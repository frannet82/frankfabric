// ---------------------------------------------------------------------------
// Coach voice — plays a recorded MP3 voice line while the reply is displayed.
//
// AUDIO SOURCE: the coach's voice is a fixed recording shipped as a static
// asset, public/audio/personal-trainer.mp3 (resolved through asset() so it
// carries the GitHub Pages base path). Playback goes through a single
// HTMLAudioElement created lazily on the client. This replaces the previous Web
// Speech API (window.speechSynthesis) approach.
//
...
// while the clip plays. This lets the coach's whole-group / limb motion track
// the actual voice (CoachScene already prefers loudness over its sine fallback).
// getLoudness() returns 0 when idle or muted. If AudioContext is unavailable the
// audio still plays and getLoudness() falls back to 0 (the scene then uses its
// sine wobble).
//
...
type SpeakCallbacks = {
  // Fired when playback actually begins (talking-motion window opens).
  onStart?: () => void;
  // Fired when playback ends, is canceled, or errors (talking-motion window closes).
  onEnd?: () => void;
};

const COACH_VOICE_SRC = "/audio/personal-trainer.mp3";
...
export class CoachVoice {
  ...
  // Smoothed loudness, eased toward the raw RMS each read for stable motion.
  ...
  private ensureAudio(): HTMLAudioElement | null {
    ...
    const el = new Audio(asset(COACH_VOICE_SRC));
    // Same-origin static file, so no crossOrigin needed. Preload so the clip is
    // ready to start promptly on the send gesture.
    el.preload = "auto";
    this.audio = el;
    return el;
  }
  ...
  /**
   * Stop any in-flight playback and close the talking-motion window. Pauses the
   * element, resets to the start, detaches the in-flight finish handler, and
   * clears the playing flag / smoothed loudness. Fires the pending onEnd exactly
   * once so an explicit stop closes the window.
   */
  private stop(): void {
  ...
  /**
   * Play the recorded coach voice line while the reply is displayed. If muted,
  ...
  getLoudness(): number {
    ...
    // Scale up a touch so speech (which rarely hits full-scale) drives lively
    // motion, and clamp to 0..1.
    const level = Math.min(1, rms * 2.2);
    // Ease toward the new level for stable, non-jittery motion.
